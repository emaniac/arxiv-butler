...
from benchmarks.metrics import compute_correctness, evaluate, Metric
...
def simulate_conversation(user_agent: Agent, tested_agent: Agent, max_turns: int = 100) -> list[AnyMessage] | None:
...
def main(dataset_path: Path) -> None:
    ...
    for example in examples:
        ...
        trajectory = simulate_conversation(user_agent, arxiv_agent)
        example_metrics = evaluate(example, trajectory, llm_client, metrics=[Metric.CORRECTNESS])
